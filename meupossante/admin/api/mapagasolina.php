    ...
    if($result){
      if($result->num_rows > 0){
        while ($row = $result->fetch_assoc()) {
          ...
          $query = "SELECT postos.idposto, postos.nomeposto, abastecimentos.dataaba, abastecimentos.precoaba FROM abastecimentos INNER JOIN postos WHERE abastecimentos.idposto = postos.idposto and abastecimentos.idcomb = 1   and abastecimentos.idposto = " . $row['idposto'] ." order by dataaba desc limit 1;";
          ...
          if($row['idposto'] != 41){
           ...
           if($precoPosto[$row['idposto']] != 'não informado'){
            echo "content:'" . $row['nomeposto'] . "<br>Gasolina R$ " . $precoPosto[$row['idposto']] . "'\n";
          } else {
