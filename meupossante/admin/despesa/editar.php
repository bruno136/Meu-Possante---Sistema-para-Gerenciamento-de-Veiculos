...
<body>
    <nav class="orange">
        <div class="nav-wrapper">
            ...
            <ul class="right hide-on-med-and-down">
                <li><a href="despesa.php?id=<?= $idveiculo ?>" class="waves-effect waves-light btn">Voltar</a></li>
            </ul>
        </div>
    </nav>
    ...
    <h4 class="center-align " style="color: orange;">Editar Despesa (<?=$_SESSION['placa']?>)</h4>
    <div class="valign-wrapper row login-box">
        ...
        //Montar e executar consulta que recupera os valores relacionados ao ID atual
            $query = "SELECT * FROM despesas WHERE iddesp=" . $id . ";";
            $resultado = $conexao->query($query);
            if ($resultado) {
             while ($linha = $resultado->fetch_assoc()) {
                $ododesp = $linha["ododesp"];
                $valordesp = $linha["valordesp"];
                $descdesp = $linha["descdesp"];
                $datadesp = $linha["datadesp"];
                $iddesp = $linha["iddesp"];
                $idveiculo = $linha['idveiculo'];
                $localdesp = $linha['localdesp'];
            }
            ?>
            <form method="post"  action="processaeditar.php">
                <input type="hidden" value="<?= $idveiculo?>" name="idveiculo" id="idveiculo">
                <input type="hidden" value="<?= $iddesp ?>" name="iddesp" id="iddesp"/>
                <div class="card-content">
                    <div class="row">
                        <div class="input-field col s12">
                            <i class="material-icons prefix">description</i>
                            <label for="ododesp">Odômetro</label>
                            <input required type="number" value="<?= $ododesp ?>" class="validate" name="ododesp" id="ododesp" />
                        </div>
                        <div class="col s12 input-field">
                            <i class="material-icons prefix">calendar_today</i>
                            <label for="datadesp"></label>
                            <input type="date" value="<?= $datadesp ?>"  required class="validate" name="datadesp" id="datadesp" />
                        </div>
                        <div class="input-field col s12">
                            <i class="material-icons prefix">attach_money</i>
                            <label for="valordesp">Valor</label>
                            <input type="number" value="<?= $valordesp ?>" class="validate" step="0.01" min="0.01" required name="valordesp" id="valordesp" />
                        </div>
                        <div class="input-field col s12">
                            <i class="material-icons prefix">place</i>
                            <label for="localdesp">Local </label>
                            <input type="text" value="<?= $localdesp ?>" name="localdesp" id="localdesp" class="validate" required  />
                        </div>
                        <div class="input-field col s12">       
                            <i class="material-icons prefix">description</i>
                            <label for="descdesp">Descrição</label>
                            <input type="text"  value="<?= $descdesp ?>" name="descdesp" id="descdesp" required class="validate" />
                        </div>
                    </div>
                </div>
