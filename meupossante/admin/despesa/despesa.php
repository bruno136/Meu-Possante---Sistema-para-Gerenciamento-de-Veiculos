...
<head>
 ...
 <title>Despesas (<?= $_SESSION['placa'] ?>)</title>        
...
<body>    
  ...
  <main class="">
    <div class="container"> 
      <h4><a href="add.php?id=<?= $id?>"  style="color:orange;"><i class="material-icons">control_point</i>Despesas (<?=$_SESSION['placa']?>)</a></h4>
      <?php 
      include "../../conexao.php";
      $query = "SELECT * FROM despesas WHERE idveiculo = '$id' ORDER BY datadesp DESC;";
      $resultado = $conexao->query($query);
      if ($resultado) {
        if ($resultado->num_rows > 0) {
          while ($linha = $resultado->fetch_assoc()) {
           echo "<div><div class='row'>";
           echo("<div class='atributos col s11 m5 xl3'><b>Odômetro: </b>" . $linha["ododesp"] .  " km</div><div class='atributos col s11 m5 xl2'><b>Valor: R$ </b>" . str_replace('.', ',', $linha['valordesp']) ."</div><div class='atributos col s11 m5 xl2'><b>Data: </b>" . date("d/m/Y", strtotime($linha['datadesp'])) . "</div><div class='atributos col s11 m5 xl3'><b>Local: </b>". $linha['localdesp'] . "</div><div class='atributos col s11 m10 xl8'><b>Descrição:</b>" . $linha["descdesp"] . "</div><a href='editar.php?id=" . $linha["iddesp"] . "&idveiculo=".$linha["idveiculo"]."'><div class='atributos col s3 offset-s2 m2 offset-m2 xl1 center-align'><i style='font-size: 16px;' class='material-icons'>edit</i></div></a><a href='#' onclick='removerInfo(" . $linha["iddesp"] .", " .$id . ")'><div class='atributos col s3 offset-s1 m2 offset-m2 xl1  center-align'><i style='font-size: 16px;' class='material-icons'>delete</i></div></a>");
           echo "<div class='divisao col s11 m11 xl10  '></div></div></div>";
         }
       } else {
        echo "<div class='center'><p>Nenhum registro encontrado!</p><img src='../../img/atencao.png' /></div> ";
      }
    } else {
      die("Erro!");
    }
