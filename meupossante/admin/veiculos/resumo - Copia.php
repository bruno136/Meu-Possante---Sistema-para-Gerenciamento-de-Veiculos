...
$query = "SELECT valoraba, dataaba, odoaba FROM abastecimentos WHERE idveiculo=" . $id . " ORDER BY dataaba DESC ;";
$resultado = $conexao->query($query);
if($resultado){
...
<body>    
  ...
  <div class="container">
    <div>
      <h4 style="color:orange;">Resumo (<?=$placa?>)</h4>
      <h6 class="center">
        <?php 
    ...
    <h5 style="color:orange;">Resumo do Veículo</h5>
    <table>
     <tr>
    ...
    <tr>
      ...
      <table>
        ...
        <tr>
          <td>
          	<?php 
          		echo  number_format(($total - $lastaba) / $dias->format('%a'), 0, ',', '.') . " km";
          	 ?>
          </td>
          <td>
          	<?php 
      ...
      <h5 style="color:orange;">Resumo de Abastecimentos</h5>
      <table>
       <tr>
        <th>Custo Total</th>
        <th>Litros</th>
      ...
      <tr>
        <td><?= "R$ " . number_format($totalaba, 2, ',', '.') ?></td>
        <td>
          <?php 
          $query = "SELECT litro FROM abastecimentos WHERE idveiculo=" . $id . " and idcomb = 1 ORDER BY dataaba DESC ;";
          $resultado = $conexao->query($query);
          if($resultado){
            if($resultado->num_rows > 0){
              while ($linha = $resultado->fetch_assoc()){
...
<h5 style="color:orange;">Resumo de Manutenções</h5>
<table>
 <tr>
  <th>Custo Total</th>
...
<tr>	
  ...
  <td>
    <?= number_format(($totalmanu/$total)*100, 2, ',', '.') . " %" ?>
  </td>
  <td>
  	<?php 
...
<h5 style="color:orange;">Resumo de Despesas</h5>
<table>	
  <tr>
   <th>Custo Total</th>
 ...
 <tr>
  ...
  <td>
    <?= number_format(($totaldesp/$total)*100, 2, ',', '.') . " %" ?>
  </td>
  <td>
  	<?php 
