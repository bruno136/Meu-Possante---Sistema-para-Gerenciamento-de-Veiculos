...
<body>    
  ...
  <main class="">
    <div class="container">
      <h4><a href="addmanutencao.php?id=<?= $id?>"  style="color:orange;"><i class="material-icons">control_point</i>Manutenções (<?=$placa?>)</a></h4>
      <?php 
      include_once "../../conexao.php";
      $query = "SELECT * FROM manutencoes WHERE idveiculo = '$id' ORDER BY datamanu DESC ;";
      $resultado = $conexao->query($query);
      if ($resultado) {
        if ($resultado->num_rows > 0) {
          while ($linha = $resultado->fetch_assoc()) {
           echo "<div><div class='row'>";
           echo("<div class='atributos col s11 m5 xl3'><b>Odômetro: </b>" . $linha["odomanu"] . " km</div><div class='atributos col s11 m5 xl3'><b>Valor: R$ </b>" . str_replace('.', ',', $linha['valormanu']) ."</div><div class='atributos col s11 m5 xl2'><b>Data: </b>" . date("d/m/Y", strtotime($linha['datamanu'])) . "</div><div class='atributos col s11 m5 xl3'><b>Local: </b>" . $linha["localmanu"] . "</div><div class='atributos col s11 m10 xl9'><b>Descrição: </b>" . $linha["descmanu"] . "</div><a href='editar.php?id=" . $linha["idmanu"] . "&idveiculo=".$linha["idveiculo"]."'><div class='atributos col s3 offset-s2 m2 offset-m2 xl1 center-align'><i style='font-size: 16px;' class='material-icons'>edit</i></div></a><a href='#' onclick='removerInfo(" . $linha["idmanu"] . ", " . $id . ")'><div class='atributos col s3 offset-s1 m2 offset-m2 xl1  center-align'><i style='font-size: 16px;' class='material-icons'>delete</i></div></a>");
           echo "<div class='divisao col s11 m11 xl11  '></div></div></div>";
         }
       } else {
